---
id: http://www.example.org/v4/cells/cell-36
title: "Employees"
type: cell-databook
version: 1.1.0
created: 2026-07-10
...
This graph is the cell's one required `member` entry — a cell with a single `member` entry in the user's own category-cell tree always has `:Self` as that member (see YAML-6), regardless of what the cell's `subject` is — here, Alice herself. The "Employees" cell is a purely organizational category node (`cell:category: cat:Employees`) with no relationship or subject of its own beyond Alice's required membership, so this stub carries no further claims. Alice is both the claimant and the subject. It carries her given name, satisfying the `ContactInfoShape` `ctpl:EmployeesTemplateCell` sets as `cell:memberShape` — no longer deliberately empty.
