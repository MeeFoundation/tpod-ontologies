...
- A cell's **attachments** are the plain files inside its own `_cell-attachments` folder — a reserved subfolder sitting directly inside the cell's own folder, at most one per cell — and they are what travels with the cell when it's shared, shown in the app's **Attachments area**. Attachments are flat: `_cell-attachments` holds plain files only, never a subfolder and never a cell DataBook. Every *other* plain file loose in the cell's own folder is that member's own **private** file — it stays in their copy of the cell, syncs across their own devices like the rest of it, and never reaches another member. The folder is named for the cell rather than for sharing because most cells have a single member and share nothing at all; there the distinction is inert but still real, since it records what would travel if the cell were ever shared later — which is exactly the moment a wrong default would bite. No cell may be named `_cell-attachments`, and an empty one is legal, since the app creates it on demand. A subfolder found inside a cell's own folder is accordingly one of four things:

  | Subfolder | Test | Part of the cell's own content? |
  |---|---|---|
  | `_cell-attachments` | that reserved name, directly inside a cell's own folder | yes — the cell's attachments, and the only part that propagates |
