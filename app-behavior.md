...
Cells are stored on the user's device(s) or, for an organization's own `s:ServiceProvider`, on Personal Data Network (PDN) nodes hosted by that organization. No cell is ever stored by any cloud provider, or any third party of any kind, including The Mee Foundation. When a cell is shared, changes to its contents — including its own name, for a single-member cell or a cell with three or more members, or a two-member cell that also carries a tool — propagate to every member over the PDN; see [Naming, Renaming, and Sharing](#naming-renaming-and-sharing) below for what stays independent per member instead. Two pieces of what sits in a cell's folder never propagate at all: a member's own **private files** — every plain file loose in the cell's folder, as against the attachments inside its `_cell-attachments` folder (see [Filesystem Persistence](#filesystem-persistence) below) — and a `c:serviceTag`, written by one member's own service module and meaningful only in the instance that wrote it (see [Tags](README.md#tags) in README.md). The two differ in who decides: the first is the member's own choice about a file of theirs, the second a bookkeeping label they never see.

### Filesystem Persistence

A folder is a **cell** exactly when it holds a `_cell-attachments` folder directly inside it — that one reserved subfolder alone marks the folder as a cell, doing double duty as both the cell's attachments and its tree-node marker. A folder without one is simply a **regular filesystem folder**, not a cell — even if it contains nested cells of its own further down. The test is decidable from a single folder's immediate contents, with no tree walk and no recorded path to reconcile against, and it survives the user moving folders around outside the app, since the marker travels inside whatever folder it sits in. Two cells can never share a folder: a loose file in it would be ambiguously part of both, and with the marker being a folder of one fixed name, a directory can hold only one, so this is structurally impossible rather than merely forbidden. The corollary is worth knowing: deleting a cell's `_cell-attachments` folder in Finder stops the folder being a cell at all, exactly as deleting its DataBook once would have.

A cell's structured content — its members, tools, graphs, category, tags and name — lives **inside the app**, not in the folder. In this repo it is carried on disk instead, as a `.databook.md` file in each cell's folder; that file is development scaffolding, will not exist once the app ships, and is specified in [cell-databook.md](cell-databook.md#development-scaffolding). Everything in this section describes the filesystem a running v4 produces.

A cell may carry a `c:category` value (asserted as `v4.category` in the DataBook's YAML front matter, in this repo's scaffolding form). When present, it records the category concept — a `skos:Concept`, always ultimately reachable from `cat:Person` or `cat:Organization` — the cell was originally instantiated from; that concept is normally one of `cat:CategoryScheme`'s own, reached by following `skos:broader` upward, but it may instead belong to a [category extension](README.md#category-extensions)'s own scheme, in which case the path runs through that concept's single `skos:broadMatch` into the core taxonomy first and then upward by `skos:broader` as usual (`bhscat:BostonHubSociety` → `cat:Groups` → `cat:Person`) — so the three category types below are well-defined either way; this value is fixed at that point, not re-derived from the cell's folder's current name, so the folder can be freely renamed or moved without needing to update it. There are accordingly **three category types**, and the diagram below colors each cell's own folder icon by which one it is: **Person** (tan, category reachable from `cat:Person`), **Organization** (light blue, category reachable from `cat:Organization`), and **Custom** (purple, no `c:category` at all — a cell the user created without picking any existing category concept). Custom is identified precisely, not by judgment call: the cell simply carries no `c:category` value at all. In this repo's scaffolding the cell's DataBook filename also carries the literal `(custom)` disambiguator in place of a category-derived `<catType>` (e.g. `Friends(custom).databook.md` — see the [Filename Convention](cell-databook.md#filename-convention) in cell-databook.md), and the two must agree; that filename is a scaffolding convention, never the test itself.

A cell's own **name** — its filesystem folder's own name — is a separate, purely display-level choice, independent of the above: when the cell does have a category, the name may be copied verbatim from that category's own `skos:prefLabel` (e.g. a cell named "Others" whose category is `cat:Others`), or the user may give it a different name entirely (e.g. a cell named "Bob Johnson" whose category is still `cat:Others`). The folder is authoritative for it. The app keeps its own record of the cell's name in sync with the folder, never the other way around: renaming the folder — in the app, or in Finder or Obsidian — updates the app's record and then propagates to the cell's other members, and renaming in the app renames the folder. There is no independent display-name override anywhere. In this repo's scaffolding the same name is carried in the DataBook's `title:` field, which likewise mirrors the folder verbatim (integrity.md's Check 19). For a single-member cell or a cell with three or more members — or a two-member cell that also carries a tool — this name is part of the cell's own shared content: it is kept in sync and identical across every member's copy, exactly like its graphs, files, its note (a note's own text, including any links it contains, but not necessarily the resolvability of a link pointing outside the cell — see the folder note description below), chat, `c:category`, and every other field in its DataBook (see [Introduction to Cells](README.md#introduction-to-cells) in README.md). What stays independent per member is only their own tree *position* for the cell — the parent it's nested under, and so that parent's own name — since a cell carries no property recording its own tree position at all. A bare two-member cell (carrying no tool) is the one exception: there, the name itself is also independent per member, alongside tree position, rather than shared — see [Naming, Renaming, and Sharing](#naming-renaming-and-sharing) below for both this exception and how the app keeps a shared name unique within each member's own tree. The diagram renders this in the folder-name text color — **green** ("Predefined folder name") when the name matches the category's label verbatim, plain **black** ("User-defined folder name") when the user has customized it. This text color is unrelated to the DataBook box's fill color above — a cell can be tan-filled (Person) with black text (e.g. "Bob Johnson"), light-blue-filled (Organization) with black text, or purple-filled (Custom, no category at all) with black text — a Custom cell's name has no label to possibly match, so it is always black text too, never green.

<p align="center"><img src="images/folder-mapping.png" alt="How a cell maps onto a filesystem folder: its note, its _cell-attachments folder (which marks the folder as a cell), and the member's own private files"></p>

- A cell's **attachments** are the plain files inside its own `_cell-attachments` folder — a reserved subfolder sitting directly inside the cell's own folder, exactly one per cell — and they are what travels with the cell when it's shared, shown in the app's **Attachments area**. Attachments are flat: `_cell-attachments` holds plain files only, never a subfolder and never a cell DataBook. Every *other* plain file loose in the cell's own folder is that member's own **private** file — it stays in their copy of the cell, syncs across their own devices like the rest of it, and never reaches another member. The folder is named for the cell rather than for sharing because most cells have a single member and share nothing at all; there the distinction is inert but still real, since it records what would travel if the cell were ever shared later — which is exactly the moment a wrong default would bite. No cell may be named `_cell-attachments`. Every cell has exactly one, created with the cell and never removed, so an empty one is the ordinary case rather than an anomaly — it is simply what a cell with no attachments looks like, and it gives someone working in Finder or Obsidian rather than in the app a drop target that is already there, with no reserved name to type correctly. Hidden files — any whose name begins with a dot — are neither attachments nor private files wherever they sit: the app passes over them as filesystem noise (`.DS_Store`, a version-control placeholder) and never shows them in the Attachments area. A subfolder found inside a cell's own folder is accordingly one of four things:

  | Subfolder | Test | Part of the cell's own content? |
  |---|---|---|
  | `_cell-attachments` | that reserved name, directly inside a cell's own folder; every cell has one, which is what makes it a cell | yes — the cell's attachments, and the only part that propagates |
  | **descendant cell** | directly contains a `_cell-attachments` folder of its own | no — a separate node in the tree, never counted as part of its ancestor's content even though it physically sits inside the ancestor's folder |
  | **pass-through directory** | no `_cell-attachments` of its own, but a descendant cell nested somewhere beneath (see integrity.md's Check 11) | no |
  | **private content** | no cell anywhere beneath it | yes, but never propagates |

  Private content may nest freely, however the member likes: the flatness above is about what travels, and nothing that stays put needs it. Telling a pass-through from a private folder is the one judgment here that isn't decidable from a single folder's own contents, but it never affects the tree — the cell/not-cell test stays strictly local and unchanged, and the deeper look only picks a label between two kinds of not-a-cell. The Attachments area lists both sets: the attachments plainly, and the private files badged (🔒) under their own divider, keeping whatever folder structure they have on disk rather than being flattened in alongside the attachments. A file is attached or made private by dragging it across. On narrow/mobile layouts the area shows only its 📎 icon, with no text label, since "Attachments" doesn't fit.
- A cell's folder holds no file carrying its structured content: that lives inside the app. In this repo it is carried on disk instead, as a `.databook.md` file whose filename matches its own folder's name verbatim plus a `(<catType>)` parenthetical where that isn't already redundant, and whose `title:` field is exactly the folder's own name. That file is development scaffolding and will not exist in a running v4 filesystem — see [Development Scaffolding](cell-databook.md#development-scaffolding) in cell-databook.md. A cell's `id`, by contrast, is real at runtime: flat, opaque and independent of the folder name, derived at creation (see [Cell Id](cell-databook.md#cell-id)), and a sequential `cell-<NN>` value in this repo's own example data (see integrity.md's Check 9), the same pattern graph ids already use. The cell's note cites it, which makes the note the one file on disk that references app-internal state by id.
- Another special file acts as the cell's **note**, shown in the app's Note area. A cell has exactly one note, never more — this so-called *folder note* is stored as a single file named `X.md` inside the `X` folder, and a folder can hold no second `.md` file playing this role. Using the same name as the folder matches the convention used by PKM (Personal Knowledge Management) tools such as Obsidian (using the Folder Notes plugin), Logseq, Foam and others — including the same freeform, vault-wide linking these tools support: a folder note may link to any other note anywhere in the user's tree of cells, not just within its own cell. Once a link's target has actually been instantiated as a cell (see [Wikilink-Triggered Cell Creation](#wikilink-triggered-cell-creation) below for the one case where it hasn't been yet), the link resolves by that target cell's own stable, globally unique id (see [Cell Id](cell-databook.md#cell-id)) — not by matching its display text. Concretely, a resolved link is written using the alias syntax these same PKM tools already support — `[[7f3a9c2e5d41b08f…|foo]]`, id before the pipe, display text after — rather than inventing new syntax; an unresolved link with no target yet (see below) is simply `[[foo]]`, no pipe at all. A link's own display text travels with the cell's note when the cell is shared, but resolution never falls back to text-matching. This matters because cell names are only unique among siblings, not globally (see [Naming, Renaming, and Sharing](#naming-renaming-and-sharing) below) — a recipient's own tree can easily contain an unrelated cell that happens to share a display name with the intended target, and id-based resolution is what keeps a link from ever silently jumping to that wrong cell instead. What id-based resolution can't fix is the linked-to cell itself not traveling with the note: if the target lies outside this cell's own boundary (i.e. it isn't part of this cell's own Attachments-area content or its own folder note) and the recipient doesn't independently have a cell carrying that id — because it was never shared with them, or they organize their tree completely differently — the link simply won't resolve on their side; it fails closed (inert, shown as unresolved) rather than open (silently pointing somewhere else). This is an accepted limitation of cross-cell linking, not a defect. That boundary now has an inner edge as well: a link whose target is one of the member's own private files resolves in their own copy and fails closed on every other member's side, since a private file never travels — the same inert-rather-than-wrong behavior, for the same reason.
...
An attachment is a plain file held in the cell's own `_cell-attachments` folder, shown in the app's Attachments area (see [Filesystem Persistence](#filesystem-persistence) above). An attachment is **immutable**: once added, its content is never updated in place by any role, so a correction means deleting it and adding the corrected file. Vladimir and Sergey call this an *immutable document*, a PDF being their example.

Every plain file loose in the cell's own folder instead is that member's own **private** file, which no other member ever receives. Every row below reads the same for one, with a single difference: both *another member's* rows become `n/a`, since no other member ever has one of this member's private files to read or delete. A service reaches exactly what its principal reaches — its access is its principal's access (see [Cell Interface](#cell-interface) below) — so a member's own agent or backup service sees their private files, and a service acting for another member never does.
...
Two further capabilities move a file across the boundary between the two sets. Moving one's own private file into `_cell-attachments` is just *Add own attachment* above; the reverse direction is its own capability:
...
- **Make own attachment private** — move an attachment one added oneself out of `_cell-attachments` and back among one's own private files. Every other member sees this as a deletion, which every role may already do to an attachment of their own.
...
The new cell carries no `c:category` at all — the user didn't pick any existing category concept, so this is the same **Custom** case described in [Filesystem Persistence](#filesystem-persistence) above (purple folder-icon fill; `foo(custom).databook.md` in this repo's scaffolding). It is a single-member cell, created by and with `:Self` as its sole member, following the same minimal-stub-graph pattern used elsewhere for a cell with nothing substantive to say yet (e.g. the bare given-name claim in Sophia Walker's Health & Wellness cell or her primary care physician's cell — see example.md). Its own folder note starts out blank, ready for the user to fill in.
...
**Any cell can gain a tool this way, as many as the user likes.** The action is not restricted to categories whose template declares one, and the picked template does not have to be one the cell's own category declares: the dialog offers the full list of shapes regardless, and whatever the user picks is stamped onto the new graph as-is. A category's `c:TemplateCell` therefore has no authority over a hand-added tool, and a category that declares no tool offers no shape hint either — integrity.md's Check 27 deliberately does not check such a tool's graphs against the category at all.

Nothing caps how many tools a cell may hold. What the cell's own member count does cap is the number of graphs beneath any one tool — one per claiming member (Check 25) — which applies equally to a tool the template declared and one the user added. Alice's Immediate Family cell for her daughter Sophia (cell-12) is the worked example of the manual path: `cat:ImmediateFamily` declares no tool, and Sophia has no instance of the app and so cannot be one of the cell's members, so Alice adds a Form tool about her by hand, picking Contact Info herself.
...
Every `s:Service` reaches a cell through one and the same surface, the **Cell Interface**: the read and write operations over a cell's [note](#note-area), [attachments](#filesystem-persistence), [chat](#chat-area), and `c:member`/tool claims. There is no side door — no service reads a cell's files off disk or writes to another member's claims directly, and none gets an API of its own.
...
- **Read access is unrestricted across the cell** — every member, human or service, can read the note, every member's and every topic's claims, and attachment metadata. "Across the cell" means the cell's shared content: a member's own private files are not part of it, and a service reaches them only when it acts for that member, its access being its principal's access.
...
4. The response is applied as one atomic turn, all under the agent's own existing member-level write rights:
   ...
   - optionally, a direct edit to the shared note, and/or a new attachment (e.g. a fetched photo of a hotel or landscape) added to the cell's flat `_cell-attachments` folder.
