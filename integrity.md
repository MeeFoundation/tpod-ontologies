...
Many of these checks read the filesystem — the tree under `example/Cells/`, the `*.databook.md` files in it, and the PNGs under `example/graphs/images/`. All of that is **development scaffolding**: V4 persists no data in the user's filesystem at all (see [storage.md](storage.md)), and the on-disk tree exists only so that real cell content can be carried, validated and diagrammed before there is an app to hold it. Checks 6, 7, 8, 11, 19, 20, 35 and sub-checks 10a, 10b, 10e and 10i are scaffolding checks in that sense: they keep this repo internally consistent and say nothing about how a running v4 stores anything. The rest test the RDF content itself, and survive unchanged whatever the app does.
