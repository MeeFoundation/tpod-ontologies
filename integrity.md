...
- **38c** — `Calendar`, `Canvas`, `Contacts` and `Map` carry no arrows: `cell.ttl` defines no property with any of them as its domain, and they inherit none, since `Tool` carries none either. All four are drawn all the same, so the extension point reads as plural rather than as a `Form` that happens to be abstract's only child.
