...
- **10d — Numbered graph circles/squares have matching embedded graphs**: Every numbered graph circle or square (e.g. `[10]`, `[17]`) shown in a diagram must correspond to a `v4.member` entry or a tool's own graph (equivalently, a `### Graph NN` body section) in some cell-databook under `example/Cells/` whose id contains that number (e.g. `(10)`, `(17)`).
...
**Check 21 — `:Self` must be a member of every cell in the user's own tree**: A cell-databook under `example/Cells/` — the user's own instance tree — can only ever have gotten there one of two ways: (1) the user created it themselves, in which case they (`:Self`) are trivially a member, or (2) someone else shared it with the user, in which case the share necessarily made `:Self` a member (a cell can't be "shared with" someone without them becoming a member of it). Either way, `:Self` must be one of the cell's active members — i.e. `:Self` must be the `cell:subject` of at least one of that cell's `member` — for **every** cell in `example/Cells/`, regardless of how many members the cell has or what the cell's derived subject (Check 18) is. This is strictest for a cell with only one `member` entry: that entry's subject must be `:Self`, full stop — never a tool's own `cell:formTopic` (see Check 18's placement rule above), even when the cell's derived-from-tool subject is a third party (e.g. `Jane_Starostina`, `Sophia_Walker`, `Ginger`) and no other graph happens to exist yet. Cells with more members have more room, so `:Self` just needs to be one of the members alongside whichever other real members the cell has (already satisfied by every existing example, e.g. Bob Johnson, Fred Flintstone, Medical Appointment, Boston Hub Society). This is not itself an OWL/SHACL-expressible constraint (same reasoning as Check 18 — it requires dereferencing each `member` value's own `subject`, not just counting or matching cardinalities), so it's checked here instead. Run:
...
for f in glob.glob('example/Cells/**/*.databook.md', recursive=True):
    ...
    for g in graphs:
        if not isinstance(g, dict) or not g.get('shape'):
            continue
        raw, gid = g['shape'], g['id']
        ...
        for shape in templates:
            ...
            if not found:
                violations += 1
                print(f"VIOLATION {f}: graph {gid} declares shape {shape!r} (resolves to {resolved!r}) but no individual in its own turtle block is asserted rdf:type {resolved!r}")
print('All shape-declaring graphs assert a matching rdf:type.' if violations == 0 else f'{violations} violation(s) found.')
```
...
# --- cat-templates.ttl: category local name -> {'member': [...], 'tool': [...]} shape CURIEs ---
category_shapes = {}
for block in re.split(r'\n\n(?=ctpl:)', read('cat-templates.ttl')):
    ...
    def shape_values(prop, block):
        # Handles both a single value (cell:memberShape pshapes:X ;)
        # and a comma-separated list (cell:memberShape pshapes:X , pshapes:Y ;) —
        # cell:memberShape/cell:formShape are 0..N, so a template may
        # carry more than one value under one predicate.
        # The value list runs to its own line's terminator, which may be ';',
        # '.', or ']' — the last when the property sits inside a bracketed
        # declared-tool blank node — so strip whichever ended it.
        m = re.search(prop + r'\s+(.+)$', block, re.M)
        if not m:
            return []
        return [v.strip() for v in m.group(1).rstrip(' ;.]').split(',')]
    # A declared tool is a bracketed blank node carrying cell:formShape;
    # its absence is exactly what says the category declares no tool, so there
    # is no separate boolean to read.
    declares_tool = 'cell:declaresTool' in block
    category_shapes[cat_local] = {
        'member': shape_values('cell:memberShape', block),
        'tool': shape_values('cell:formShape', block),
        'declares_tool': declares_tool,
    }
...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    def entries_for(field):
        if field == 'member':
            raw = v4.get('member') or []
            return raw if isinstance(raw, list) else [raw]
        out = []
        tools = v4.get('tool') or []
        for tool in (tools if isinstance(tools, list) else [tools]):
            graphs = tool.get('graph') or []
            out += graphs if isinstance(graphs, list) else [graphs]
        return out
    ...
    for field in ('member', 'tool'):
        ...
        for entry in entries_for(field):
            gid = entry.get('id')
            raw = entry.get('shape')
            form_shapes = raw if isinstance(raw, list) else ([raw] if raw else [])
            prop = 'memberShape' if field == 'member' else 'formShape'
            for shape in form_shapes:
                if not allowed:
                    violations += 1
                    print(f"VIOLATION {path}: {field} graph {gid} declares shape {shape!r} but cell's TemplateCell has no cell:{prop} value(s)")
                elif shape not in allowed:
                    violations += 1
                    print(f"VIOLATION {path}: {field} graph {gid} declares shape {shape!r}, not among cell's TemplateCell's cell:{prop} value(s) {sorted(allowed)}")
print("All member/tool shape: values match their TemplateCell's shape value(s)." if violations == 0 else f'{violations} violation(s) found.')
```
...
for block in re.split(r'\n\n(?=ctpl:)', read('cat-templates.ttl')):
    ...
    category_shapes[cat_local] = shape_values('cell:memberShape', block)
...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    for entry in entries:
        ...
        if missing:
            violations += 1
            print(f"VIOLATION {path}: member graph {gid} is missing required c:shape value(s) {sorted(missing)}")
print("All c:member graphs carry every c:shape value their category's TemplateCell requires." if violations == 0 else f'{violations} violation(s) found.')
```

If a violation is found: add the missing `shape:` value(s) to the graph's own `v4.member[]`/`v4.tool[].graph[]` entry, and verify (per Check 26) that the graph's own individual — usually its `:Self` claim, per the required minimal `GivenName` stub — actually satisfies the shape's constraints; if it doesn't yet, add the missing content rather than just the template value.
...
missing = sorted(set(concepts) - existing)
violations = 0
for m in missing:
    violations += 1
    print(f"VIOLATION: cat:{m} ({concepts[m]!r}) has no matching cell:TemplateCell in cat-templates.ttl")
print("Every category concept, top concepts included, has a matching TemplateCell." if violations == 0 else f'{violations} violation(s) found.')
```
...
- **green (Other)** — every other claimant: another `p:Person`, an `o:Organization`, or any `service:Service`, whether it is an `s:AgentService` leaf or an `s:ServiceProvider`, and whether the graph is a `member` or a `topic`.
...
**Check 34 — no diagram still carries the retired "Delegate" legend swatch**: The 12 example diagrams plus `images/representative-cells.png` and `images/folder-mapping.png` each carry their own copy of the legend, so a change to it has to be applied fourteen times by hand. That legend is now five separate titled boxes — "Name", "Category", "Claimant", "Tool", "Members" — rather than the single "Key" box it once was (see Check 15 for the full box-by-box contents); the ones that matter here are **Claimant**, holding two claim-fill swatches (green "Other", dashed "Self"), **Tool**, holding one shape swatch (square "Form"), and **Members**, holding two (circle "Human", octagon "Service"); it no longer holds the gray "Delegate" swatch that a third claim-fill state once needed (see Check 15 and Check 33 — claim fill is now a two-state self/not-self fact, and a member's identity type is carried by shape instead). A diagram left on the older key is mechanically detectable, because that swatch is the only place a filled `(235, 235, 235)` block of any size appears: the new key uses no gray at all, and the incidental gray in these PNGs is antialiasing a few dozen pixels in size, far below the threshold below. This catches the stale-legend case that Check 33 cannot — Check 33 compares fills against the DataBooks and so passes happily on a diagram whose *shapes* are all correct but whose *legend* still advertises a retired state. Requires `pip install pillow`. Run:
...
**Check 37 — `cell.ttl`'s `cell:ServiceTag` matches `images/cell-ontology/tag.png`**: The service-tag value node has its own diagram rather than sitting inside `cell.png`, since it takes no part in the `Cell` hierarchy at all. `ServiceTag` (concrete but hidden, green) is drawn as a standalone box carrying three arrows — `tagNamespace`, `tagKey` and `tagValue` — each to an `xsd:string` box at `1..1`:
...
- **37c** — no `serviceTag` arrow is drawn *into* `ServiceTag` here; that arrow belongs to `InstanceCell` in `cell.png` (Check 12), which shows `c:ServiceTag` only as a target box.

**Check 38 — `cell.ttl`'s `cell:Tool` hierarchy matches `images/cell-ontology/tool.png`**: The tool hierarchy has its own diagram for the same reason `tag.png` does — `cell:Tool` has no `rdfs:subClassOf` relation to `cell:Cell`, so it is not part of that hierarchy and is only pointed at from it. `Tool` (abstract, blue) sits at the top with its four concrete children `Calendar`, `Canvas`, `Map` and `Form` (all black) below it:
