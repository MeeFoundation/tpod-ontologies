...
4. Re-read the prose that names facts the ontology settles, now in two places:
   ...
   - **`app-behavior.md`'s [Form Types](../../app-behavior.md#form-types)** — its closing sentence names the four tool kinds with no content model (`cell:Calendar`, `cell:Canvas`, `cell:Contacts`, `cell:Map`), which `cell.ttl` settles. That subsection is not generated and holds no table; check it only when the tool kinds change.
