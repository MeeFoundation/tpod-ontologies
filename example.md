...
The cell also carries the completed form itself as an **attachment** — `Personal Page for BHS 2026 Directory (Alice Walker).pdf`, sitting in the cell's own `_cell-attachments` folder, which is all an attachment is. Its answers are the same ones [graph 14](<example/Cells/Groups/Boston Hub Society/Boston Hub Society.databook.md#graph-14>) carries as RDF, field for field, so the cell holds the paper and the data side by side: the questions and Alice's handwritten answers in its Attachments area, the same answers structured and validated in its member graph.

Beside it, this cell is also the example tree's one worked case of the other half of that rule. Alice's `Directory Drafts` folder — a subfolder of the cell's own folder with no cell anywhere beneath it — holds `answers-draft.md`, the notes she made while filling the form in: who to ask before naming them as her sponsor, which two of the society's 19 industry options she settled on, and three attempts at the goals question she was not happy with. Because it sits loose in the cell's folder rather than in `_cell-attachments`, it is private to Alice: it syncs to her own devices with the rest of her copy of the cell, and neither Bob nor `:BHS_Service` ever receives it, though both are members of this same cell. The attachment and the draft are one folder apart and on opposite sides of what propagates — see [Filesystem Persistence](app-behavior.md#filesystem-persistence) in app-behavior.md.
