...
The tree's purely organizational **scaffold cells** — the branch nodes whose real content lives in their own leaf cells — carry graphs too, and they are listed here rather than mixed in above. Each carries the `:Self` stub `member` entry Check 21 requires of every cell, and most carry a deliberately-empty tool graph as well. These are the graphs the 12 diagrams deliberately do **not** draw: a scaffold cell's box shows no graph shapes at all, a visual simplification that keeps the diagrams uncluttered (integrity.md's Check 10h), which is a decision about the drawing only and never about whether the graph exists or is documented.
