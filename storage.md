...
V4 persists **no data in the user's filesystem**. Every cell and everything in it lives in a
...
[cell-databook.md](cell-databook.md) and [CLAUDE.md](CLAUDE.md), which describe the model itself and
point here rather than restating any of this.

## The decision

A cell's entire content is app-internal: its members, tools, graphs, category, tags, name and id,
together with its note, its attachments, its chat, and a member's own private files. None of it is
written to the filesystem, in any form, at any point. There is no folder representing a cell, no
file representing its note, and no directory holding its attachments.

The store sits alongside the cell's membership store — the append-only event log whose first entry
is the cell's founding event (see [Cell Id](cell-databook.md#cell-id) in cell-databook.md) — rather
than parallel to it. As before, no cell is ever stored by any cloud provider, or any third party of
...
- **What a cell is.** A cell is a unit the app manages, identified by its own stable id. Nothing
  marks it in a filesystem, because it is not in one. The reserved `_cell-attachments` folder that
  once served as that marker is a scaffolding convention only (see below).
- **Tree position.** `c:Cell` still asserts no tree position, exactly as before — but the reason is
  now that a cell's parent is per-member state in that member's own store, never shared content.
  This is what lets two members of a shared cell each file it wherever they like without touching
  what the other sees.
- **A cell's name.** The app's own record of the name is authoritative outright; there is no folder
  for it to mirror. The rules around it are unchanged — sibling uniqueness with auto-suffixing, and
  which cells share a name across members versus keeping one per member (see
...
- **The note.** Still exactly one per cell, still Markdown, still supporting `[[wikilinks]]` that
  resolve by target cell id. It is no longer a file, so the folder-note convention that once tied it
...
|---|---|
| Every `*.ttl` ontology — `persona.ttl`, `cell.ttl`, `category.ttl`, `cat-templates.ttl`, `service.ttl`, `organization.ttl`, and every `other/`, `persona-ext/` and `category-ext/` peer file (possibly expressed differently in the app) | Every `*.databook.md` file, and the whole format specified in [cell-databook.md](cell-databook.md) |
| Every `**/shacl/*.ttl` shapes file | The `example/Cells/` tree, and every folder in it |
| The concepts the ontologies model — cells, members, owners, tools, graphs, categories, tags, notes, attachments, chat | Cells as folders; the `_cell-attachments` marker; folder notes named after their folder; private files as loose files; pass-through directories; the filename convention |
...
this repository — see [Development Scaffolding](cell-databook.md#development-scaffolding) in
cell-databook.md for the detailed account, and
