...
- **`c:attachment`** — a cell's flat set of file attachments, shown in the app's **Attachments area** (📎), 0..N. Attachments are the plain files inside the cell's own `_cell-attachments` folder: a reserved subfolder sitting directly inside the cell's own folder, at most one per cell, holding plain files only — never a subfolder and never a cell DataBook. They are the part of a cell's folder that travels with it when the cell is shared, and they are flat, like email attachments, on disk and in the model alike. The folder is named for the cell rather than for sharing because most cells have a single member and share nothing at all; there the distinction is inert but still real, recording what would travel if the cell were ever shared later. Every **other** plain file loose in the cell's own folder — and any subfolder with no cell anywhere beneath it, which the member may nest however they like — is that member's own **private** file: shown in the same Attachments area, marked as private, but never reaching another member. The cell's own Markdown folder note is neither, keeping its special role and its own **Note area**. The two remaining kinds of subfolder are not the cell's content at all: a **descendant cell** (a nested folder that is itself a cell, holding its own cell DataBook: a separate node in the tree of cells, never counted as part of its ancestor's content even though it physically sits inside the ancestor's folder) and a bare **pass-through directory** with no DataBook of its own but a cell somewhere beneath it (a folder without a matching cell DataBook is simply a regular filesystem folder, not a cell — even if it contains nested cells of its own — existing only to reach a descendant cell nested deeper still; see integrity.md's Check 11). For a worked pair — one file attached, one kept back, in the same shared cell — see [example.md](example.md#boston-hub-society).
