...
- **`pod:userTag`** (see [InstancePod](#instancepod) above) is typed freely by the user — no list to choose from, no approval step, no restriction beyond being non-empty text. Alice tags her cat's three pods — `Ginger`, its `Medical` pod, and its `Care & Feeding` pod — **Ginger**, so one search collects everything about the cat regardless of where in the tree each pod sits; that those pods happen to be nested under one another is incidental, and the same tag would gather them just as well if they were scattered.
...
- **`pod:declaresTool`** — names the tools a pod of this category is created with, each value a `pod:Tool` node carrying its own `pod:formShape` and nothing else — e.g. `ctpl:MedicalAppointmentTemplatePod` (which also carries `pod:category podcat:MedicalAppointment`) declares one `pod:Form` carrying `mashapes:MedicalAppointmentRecordShape`. An `owl:ObjectProperty`, domain `pod:TemplatePod`, range `pod:Tool`, zero or more values, no fixed upper bound.
  ...
  Presence or absence is the whole of what a template says about tools: there is no boolean flag declaring whether a pod of this category carries a tool, because declaring the tool already says it, and a second statement could only ever disagree with the first. 18 of the 106 templates declare a tool today, each a `pod:Form` — 14 of them naming a real document, account, or organization shape (`ctpl:PassportTemplatePod`/`SSNTemplatePod`/`BirthCertificateTemplatePod`/`DriversLicenseTemplatePod` — Alice's own government documents; `ctpl:MedicalAppointmentTemplatePod` — Sophia; `ctpl:PetMedicationsTemplatePod`/`ctpl:PetProfileTemplatePod` — Ginger; `ctpl:VehicleProfileTemplatePod` — the RAV4; `ctpl:CompaniesTemplatePod`/`ctpl:BankingPaymentsTemplatePod` — a company's own service account, plus a bank's debit card/checking account; `ctpl:HomeTemplatePod` — a residence, current or previous; `ctpl:TripsTemplatePod` — a trip itinerary; `ctpl:TravelProviderTemplatePod` — the loyalty/service account held with a travel provider, `sashapes:ServiceAccountShape`; `ctpl:GroupsTemplatePod` — a society or association a person belongs to, whose own `o:Organization` profile is the tool's topic; and `ctpl:OrganizationTemplatePod` — the bare `podcat:Organization` top concept, standing for an organization such as an employer, whose own profile is likewise the topic), plus 3 more naming a shape for a more modest pattern, every property optional — `ctpl:HealthWellnessTemplatePod` (Sophia's physical characteristics — height, eye color, hair color), `ctpl:PrimaryCarePhysicianTemplatePod` (a physician's medical specialty — Dr. Jane Starostina's own `persona:specialty "Endocrinology"`), and `ctpl:PetsCareAndFeedingTemplatePod` (a pet's own identifying properties, reused from `pets:Pet`, every one optional here). The other 88 declare none: `ctpl:PeopleTemplatePod` and its four direct `skos:broader` children, every other templated category (each purely organizational, with no document, account or record content of its own — see TTL-6 in integrity.md, which requires every `pod-categories.ttl` concept except `podcat:Person`/`podcat:Organization` to have a template), plus `ctpl:UserDefinedTemplatePod` (the no-category fallback, likewise with nothing beyond a contact-info member).
...
- **BFO semantics**: changing the range of `BFO_0000115` to a DataBook document IRI (e.g. `<http://www.example.org/tpod/graphs/graph-07>`) would be a semantic error — the range of `has member part` must be a continuant (a person or group), not a document.
...
- **Tooling maturity**: annotating the triple with RDF-star (`<< :Alice_Family_Network BFO_0000115 :Sophia_Walker >> tpod:inContext <...>`) is a valid future option, but is not yet supported by Protégé and remains non-standard.
...
- **`shacl/persona-shacl.ttl`** — SHACL constraint rules for all `p:Person` individuals across all graphs. Validates properties including:
  ...
  - `:HealthWellnessShape` — the `pod:formShape` of the tool `podcat:HealthWellness` declares — targets `p:Person` directly. Formalizes Sophia Walker's own physical-characteristics graph: height (a CCO Height quality plus its Ratio Measurement ICE), eye color, and scalp hair/hair color (self-contained, duplicating `:ScalpHairShape`/`:HairColorShape`'s own constraints inline rather than deferring to them) — every property optional
...
- `s:ChatGPT` — OpenAI's assistant, invited by a member to collaborate inside a pod (Alice's travel agent). Carries no `s:providedBy`: the class already names the vendor, and OpenAI is not a party to the pod's relationship.
