...
Not to be confused with a *pod's* own subject — see [The `v4` Block](pod-databook.md#the-v4-block) below for how the two relate; a pod has no `pod:subject` value of its own.
...
The tree below is the taxonomy the app ships with. An organization can add a category of its own without it being added here, by publishing a **category extension** — its own SKOS concept scheme, linked into this tree by `skos:broadMatch`. See [Category Extensions](#pod-category-extensions).
