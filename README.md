...
Reusable class-level templates (`cat-templates.ttl`) are the exception: each is typed solely `pod:TemplatePod`, never `pod:InstancePod` — the two are disjoint, so a template pod carries no member composition of its own.
...
Each pod's fill color and pod name text color follow a display convention rooted in the pod's own DataBook — see [Pod Contents](app-behavior.md#pod-contents) in app-behavior.md for the full rules. In short: tan fill for `People`/`Bob Johnson`/`BHS`/`Medical Appointment` (Person-rooted category), light blue for `Employee` (Organization-rooted category), purple for `Friends` (no category at all, Custom); `Bob Johnson`'s category is `cat:Others`, so its name doesn't match the label and is shown in black text, while `People`'s name matches its own category's label and is shown in green. This is purely a display choice about the pod's own DataBook box and name, not a separate RDF property.
...
A category's template pod (`cat-templates.ttl`) may also carry validation metadata declared in the paired per-template `*-shacl.ttl` file. This metadata lives on the class-level template only.
...
**`pod.ttl`** — The Pod ontology, defining:
  ...
  - *Object properties*: `pod:category` (domain `pod:Pod`, range `skos:Concept` — the category concept this pod was originally instantiated as, else nil; fixed at creation, not re-derived from the pod's current name; at most one value; asserted on both kinds of pod — a real `pod:InstancePod` and, for a templated concept, the `pod:TemplatePod` it was cloned from); `pod:memberShape`/`pod:formShape` (domain `pod:TemplatePod`, zero or more values each — see [TemplatePod](#templatepod) above); `pod:creator`/`pod:owner`/`pod:member` (domain `pod:InstancePod` — every pod in a user's own tree is typed `pod:InstancePod`, never `pod:TemplatePod`, so every such pod carries all three; the reusable class-level templates in `cat-templates.ttl` are typed `pod:TemplatePod` instead and carry none of them); `pod:tool` (domain `pod:InstancePod`, range `pod:Tool`, zero or more values) and `pod:formGraph` (domain `pod:Form`, range `pod:FormGraph`, at least one on a live form tool); `pod:declaresTool` (domain `pod:TemplatePod`, range `pod:Tool`, zero or more values) and `pod:formShape` (domain `pod:Form`, range `sh:NodeShape`, zero or more values, carried only in the declared-tool role); `pod:serviceTag` (domain `pod:InstancePod`, range `pod:ServiceTag`, zero or more values, no fixed upper bound). A pod stores no shape of its own — a `pod:InstancePod`'s validation shape is derived via a reverse lookup on its own `pod:category` value (see [InstancePod](#instancepod) above), never stored.
...
- **`shacl/persona-shacl.ttl`** — SHACL constraint rules for all `p:Person` individuals across all graphs. Validates properties including:
  ...
  - `:HealthWellnessShape` — the `pod:formShape` of the tool `cat:HealthWellness` declares — targets `p:Person` directly. Formalizes Sophia Walker's own physical-characteristics graph: height (a CCO Height quality plus its Ratio Measurement ICE), eye color, and scalp hair/hair color (self-contained, duplicating `:ScalpHairShape`/`:HairColorShape`'s own constraints inline rather than deferring to them) — every property optional
  - *Social Network*: sub-groups (via `has part`) must be Social Networks; members (via `has member part`) must be `p:Person` instances
  - `:PrimaryCarePhysicianShape` — one of the two `pod:formShape` values on the tool `cat:PrimaryCarePhysician` declares, alongside `pshapes:ContactInfoShape` — targets `p:Person` directly, since a physician has no reified document individual of their own. One optional property: `p:specialty` (e.g. Dr. Jane Starostina's own "Endocrinology"). Since both shapes target `p:Person`, Dr. Starostina's own tool graph (graph-25) is validated as a contact-info profile in addition to her specialty
...
- **`other/shacl/pets-shacl.ttl`** — SHACL shapes for pet identity, pet medication, and care & feeding graphs, each directly linked from the tool its `cat-templates.ttl` template pod declares, via that tool's `pod:formShape`:
  ...
  - `:PetsCareAndFeedingShape` — the `pod:formShape` of the tool `cat:PetsCareAndFeeding` declares — targets `pets:Pet` individuals directly, mirroring `:PetShape`'s own property list, but with every property optional, including `name`/`hasSpecies` (required by `:PetShape` itself).
...
- **`other/shacl/vehicles-shacl.ttl`** — SHACL shapes for vehicle identity graphs, directly linked from the tool its `cat-templates.ttl` template pod declares, via that tool's `pod:formShape`:
...
- **`other/shacl/identity-documents-shacl.ttl`** — SHACL shapes for the three document classes, each directly linked from the tool its `cat-templates.ttl` template pod declares, via that tool's `pod:formShape`:
...
- **`other/shacl/service-accounts-shacl.ttl`** — `:ServiceAccountShape` (`pod:shape sashapes:ServiceAccountShape`), linked from the tools `cat:Companies`'s and `cat:BankingPayments`'s template pods declare, via their `pod:formShape`. Enforces: exactly one `has user handle` (username) and one `sa:hasPassword`; `has service name`, `has service URI`, and `sa:loyaltyProgramID` each optional, at most one value.
...
- **`other/shacl/banking-shacl.ttl`** — two shapes, both linked from the tool `cat:BankingPayments`'s template pod declares, via its `pod:formShape`:
...
- **`other/shacl/residences-shacl.ttl`** — `:ResidenceShape` (`pod:shape residenceshapes:ResidenceShape`), linked from the tool `cat:Home`'s template pod declares, via its `pod:formShape`. Enforces: exactly one address and one temporal-region interval; a linked participant (the resident, usually `:Self`) optional.
...
- **`other/shacl/itineraries-shacl.ttl`** — `:ItineraryShape` (`pod:shape itineraryshapes:ItineraryShape`), linked from the tool `cat:Trips`'s template pod declares, via its `pod:formShape`. Enforces: at least one of `rdfs:label` or `rdfs:comment`.
...
- `dp:spousePartnerName` — the spouse or partner's name. A bare string rather than a link to a `p:Person` or a `p:hasSocialNetwork` membership: a directory prints the name it was given. Where a real modeled relationship is wanted, that belongs in a pod of its own under `cat:ImmediateFamily`.
...
| **Primary Care Physician** | `pshapes:PrimaryCarePhysicianShape` — [`shacl/persona-shacl.ttl`](shacl/persona-shacl.ttl) | A physician's medical specialty; optional, and paired with Contact Info on the same form | `cat:PrimaryCarePhysician` |
...
| **Social Security Number** | `pshapes:SSNShape` — [`shacl/persona-shacl.ttl`](shacl/persona-shacl.ttl) | A US Social Security Number, in `NNN-NN-NNNN` form | `cat:SSN` |
| **Passport** | `idocshapes:PassportShape` — [`other/shacl/identity-documents-shacl.ttl`](other/shacl/identity-documents-shacl.ttl) | Name, date of birth, passport number and expiration date (all required); additional name, issue date, issuing country, place of birth, gender marker and photo | `cat:Passport` |
| **Driver's License** | `idocshapes:DriversLicenseShape` — [`other/shacl/identity-documents-shacl.ttl`](other/shacl/identity-documents-shacl.ttl) | Name, date of birth, license number and expiration date (all required); additional name, postal address, issuing jurisdiction and photo | `cat:DriversLicense` |
| **Birth Certificate** | `idocshapes:BirthCertificateShape` — [`other/shacl/identity-documents-shacl.ttl`](other/shacl/identity-documents-shacl.ttl) | A full name, or a given plus family name; additional name, alternate name, nickname and legal name are optional | `cat:BirthCertificate` |
| **Service Account** | `sashapes:ServiceAccountShape` — [`other/shacl/service-accounts-shacl.ttl`](other/shacl/service-accounts-shacl.ttl) | An online account — username and password (required); service name, service URI and loyalty program ID | `cat:BankingPayments`, `cat:Companies`, `cat:TravelProvider` |
| **Debit Card** | `bankingshapes:DebitCardShape` — [`other/shacl/banking-shacl.ttl`](other/shacl/banking-shacl.ttl) | Card number and expiration date (required); CVV, and a link to the checking account it draws on | `cat:BankingPayments` |
| **Checking Account** | `bankingshapes:CheckingAccountShape` — [`other/shacl/banking-shacl.ttl`](other/shacl/banking-shacl.ttl) | Exactly one account number and one routing number | `cat:BankingPayments` |
| **Residence** | `residenceshapes:ResidenceShape` — [`other/shacl/residences-shacl.ttl`](other/shacl/residences-shacl.ttl) | A place lived in — exactly one address and one temporal interval (an open-ended one meaning current), plus the resident | `cat:Home` |
| **Vehicle** | `vehicleshapes:VehicleShape` — [`other/shacl/vehicles-shacl.ttl`](other/shacl/vehicles-shacl.ttl) | Vehicle type, make, model and model year (all required); VIN, color, body type, fuel type, drive wheel configuration, odometer reading and engine specification | `cat:Vehicles` |
| **Pet** | `petshapes:PetShape` — [`other/shacl/pets-shacl.ttl`](other/shacl/pets-shacl.ttl) | A pet's name and species (required); breed, birth date, body weight, sex and spay/neuter status | `cat:Pets` |
| **Pet Care & Feeding** | `petshapes:PetsCareAndFeedingShape` — [`other/shacl/pets-shacl.ttl`](other/shacl/pets-shacl.ttl) | The same fields as Pet, every one optional, so day-to-day care instructions may identify the pet by any subset of them or none | `cat:PetsCareAndFeeding` |
| **Pet Medications** | `petshapes:PetMedicationRecordShape` — [`other/shacl/pets-shacl.ttl`](other/shacl/pets-shacl.ttl) | At least one medication, each with its active ingredients, dosage amount and administration schedule | `cat:PetsMedical` |
| **Medical Appointment** | `mashapes:MedicalAppointmentRecordShape` — [`other/shacl/medical-appointments-shacl.ttl`](other/shacl/medical-appointments-shacl.ttl) | Patient, insurance provider and policy number (required); primary care physician, insurance group number, preferred pharmacy and medical history notes | `cat:MedicalAppointment` |
| **Trip Itinerary** | `itineraryshapes:ItineraryShape` — [`other/shacl/itineraries-shacl.ttl`](other/shacl/itineraries-shacl.ttl) | A trip's plan as free text — at least a human-readable label or description, the itinerary being drafted and revised in prose rather than in fixed fields | `cat:Trips` |
| **Organization** | `oshapes:OrganizationShape` — [`shacl/organization-shacl.ttl`](shacl/organization-shacl.ttl) | An organization's own profile — website and member or employee count, alongside its name and self-description | `bhscat:BostonHubSociety`, `cat:Groups`, `cat:Organization` |
