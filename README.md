...
- **Attachments** (📎) — an optional set of one or more files, held in the cell's reserved `_cell-attachments` folder and shared with every member. Any other plain file the user leaves loose in the cell's own folder is private to them and never propagates, though both sets are shown in this same area. Some common filetypes (images, video, PDFs, Markdown) will show previews whereas others (Excel spreadsheets, Word documents, etc.) might not.
...
A cell is an atomic unit of information that the app manages for the user. This unit consists of a filesystem folder marked as a cell by the `_cell-attachments` folder directly inside it — cells nest inside cells, forming the tree.

`c:Cell` (below) still models only the content *facet* the app holds for that folder, and stores no property recording the cell's own tree position: the folder ↔ cell pairing is always one-to-one (see [Filesystem Persistence](app-behavior.md#filesystem-persistence) in app-behavior.md), so a cell's position in the tree is simply wherever its folder currently sits — moving or renaming that folder is a pure filesystem operation that never requires updating anything asserted on the cell itself.
...
- **`c:attachment`** — a cell's flat set of file attachments, shown in the app's **Attachments area** (📎), 0..N. Attachments are the plain files inside the cell's own `_cell-attachments` folder: a reserved subfolder sitting directly inside the cell's own folder, exactly one per cell — present even when it holds nothing, since an empty one is simply what a cell with no attachments looks like — holding plain files only, never a subfolder and never a cell DataBook. They are the part of a cell's folder that travels with it when the cell is shared, and they are flat, like email attachments, on disk and in the model alike. The folder is named for the cell rather than for sharing because most cells have a single member and share nothing at all; there the distinction is inert but still real, recording what would travel if the cell were ever shared later. Every **other** plain file loose in the cell's own folder — and any subfolder with no cell anywhere beneath it, which the member may nest however they like — is that member's own **private** file: shown in the same Attachments area, marked as private, but never reaching another member. The cell's own Markdown folder note is neither, keeping its special role and its own **Note area**, and a hidden file — any whose name begins with a dot — is neither either, being filesystem noise the app passes over wherever it sits. The two remaining kinds of subfolder are not the cell's content at all: a **descendant cell** (a nested folder that is itself a cell, holding a `_cell-attachments` folder of its own: a separate node in the tree of cells, never counted as part of its ancestor's content even though it physically sits inside the ancestor's folder) and a bare **pass-through directory** with no marker of its own but a cell somewhere beneath it (a folder without a `_cell-attachments` folder is simply a regular filesystem folder, not a cell — even if it contains nested cells of its own — existing only to reach a descendant cell nested deeper still; see integrity.md's Check 11). For a worked pair — one file attached, one kept back, in the same shared cell — see [example.md](example.md#boston-hub-society).
...
A **tool** is an object a cell can carry — a form, a calendar, a drawing canvas, a map — each with its own data format and its own contribution to the app's UI, sitting alongside the cell's unstructured note and its attachments. A cell carries zero or more, via `c:tool`; a cell with none is the ordinary case, not a lesser kind of cell. The ontology models only the data format; what a tool contributes to the interface is app-level, and [app-behavior.md](app-behavior.md) describes it.

<p align="center"><img src="images/cell-ontology/tool.png" alt="tool hierarchy"></p>

Four tool classes are defined. `c:Form` is the one whose format is settled: its data is graph DataBooks — one or more `c:FormGraph` values, whose fields the app renders as a form from the SHACL shape governing the tool (see [Form Fields from SHACL Shapes](app-behavior.md#form-fields-from-shacl-shapes) in app-behavior.md). `c:Calendar`, `c:Canvas` and `c:Map` carry no properties of their own yet — they are defined so the extension point is visibly plural, and so that adding a tool later is a new subclass rather than a new property on `c:InstanceCell`.

All three tool properties — `c:formTopic`, `c:formGraph` and `c:formShape` — have domain `c:Form`, and `c:Tool` itself carries none. That follows from what a tool is rather than being an oversight: each kind brings its *own* data format, so there is nothing common to hoist onto the abstract class. A form's format is graphs conforming to a shape; a calendar's would be dated entries, a canvas's a drawing surface and a map's placed features, none of them a graph and none worked out. A property that proves common once those three have formats of their own can be hoisted up then — cheap, where retracting one asserted too early is not.
...
The Service ontology models the non-human services a person invites to collaborate inside a shared cell — the second kind of first-class, member-capable participant, peer to the Persona ontology. A cell's members are only ever `p:Person` or `s:Service` individuals. An `o:Organization` is **not** member-capable: an organization participates in a cell through the `s:ServiceProvider` it provides, and otherwise reaches a cell only as a `c:formTopic` or a `c:claimant`.

<p align="center"><img src="images/service-ontology/service.png" alt="Service model"></p>

An `s:Service` is never a `c:creator` (see [Cell Ontology](#cell-ontology)) — a cell is always created by, and only ever owned by, a `p:Person` — but it can be a genuine `c:member` participant, and a `c:claimant` (see [Graphs](#graphs)) of the graphs it contributes, including a tool's own graph about whatever the cell's relationship concerns. Whatever it does outside the cell, every `s:Service` reaches the cell itself through one surface — the **Cell Interface**, the same read/write surface a human member's own UI uses, which is why an invited service needs no special-case permission logic; see [Cell Interface](app-behavior.md#cell-interface) in app-behavior.md. Each member of a shared cell can independently bring their own AI agent: when a member invites theirs in, it becomes a real member, distinct from any agent a different member separately invites.
...
- `s:AppleContacts` — the contact-sync service, which imports a member's contacts into cells and carries later edits back out. An agent service with no LLM in it at all; what makes it one is that it syncs exactly one member's address book. It is what writes the `c:serviceTag` values recording an Apple Contacts *Group* — namespaced under its own developer's domain, not Apple's.
...
`shacl/service-shacl.ttl` targets each class in turn. `:ServiceShape` (target `s:Service`) constrains `s:providedBy` and `s:actsFor` to at most one value each, an `o:Organization` and a `p:Person` respectively. `:AgentServiceShape` (target `s:AgentService`) raises `s:actsFor` to exactly one value, inherited by all three of its leaves; `:ServiceProviderShape` and `:ArcaBackupShape` each raise `s:providedBy` to exactly one value. `s:ChatGPT` and `s:AppleContacts` get no shape of their own, since they add no constraint beyond the one they already inherit. No shape forbids `s:actsFor` on `s:ServiceProvider` — since no class is disjoint from any other, that would be unsatisfiable against `:AgentServiceShape`'s own minimum for any service typed both an agent service and a provider.
